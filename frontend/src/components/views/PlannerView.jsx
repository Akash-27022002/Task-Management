...
const PlannerView = ({ tasks, selectedDate, handleDateChange, timetable, setTimetable, setTasks, onEditTask, onToggleDone, commonBlocks, setCommonBlocks, getDayBlocks }) => {
  ...
  const [viewMode, setViewMode] = useState("tasks"); // NEW toggle state
          ...
          {fullBlocks.map((b, i) => {
            ...
            return (
              <div
                ...
                onClick={() => {
                  const newLabel = prompt("Name this block:", b.label);
                  if (newLabel) b.label = newLabel;
                }}
              >
                {b.label}
              </div>
  ...
  const handleAddTimetableBlock = (day) => {
    ...
    if(utils.t2min(newBlock.start) >= utils.t2min(newBlock.end)){
       alert("Error: start can't be greater than end Time");
       return;
    }

    const existingBlocks = timetable[ttWeekKey]?.[day] || [];
    ...
    setTimetable((prev) => {
      ...
      newTt[ttWeekKey][day] = [...newTt[ttWeekKey][day], newBlock];
      return newTt;
    });
  ...
  <div className="space-y-6">
    <div className="flex items-center justify-between">
      ...
      <input
        ...
        className="bg-white/[0.06] ring-1 ring-white/10 hover:ring-white/20 text-sm px-3 py-2 rounded-md outline-none"
      />
    </div>
    ...
    {viewMode === "tasks" ? (
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03]">
            ...
            <div className="p-4 space-y-3">
              ...
              <button
                ...
                className="w-full px-3 py-2 rounded-md bg-indigo-500/90 hover:bg-indigo-500 ring-1 ring-indigo-400/30 hover:ring-indigo-300/50 text-sm font-medium text-white"
              >
                Add
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 rounded-xl border border-white/10 bg-white/[0.03] p-4 space-y-1">
          ...
          //   <DayTimeline key={day} day={day} blocks={timetable[ttWeekKey]?.[day] || []} />
          // ))

          utils.dayKeys.map((day) => {
            // Build actual date string for this week + day
            // const dateStr = utils.dateStrFromWeekAndDay(ttWeekKey, day);
            return <DayTimeline key={day} day={day}  blocks={timetable[ttWeekKey]?.[day] || []} />;
          })
          }
        </div>

        {/* Sidebar for TIMELINE mode */}
        <div className="space-y-4">
          {/* Timetable editor */}
          <div className="rounded-xl border border-white/10 bg-white/[0.03]">
            <div className="px-4 py-3 flex items-center gap-2 border-b border-white/5">
              ...
              <div className="ml-auto flex items-center gap-2">
                <select
                  value={ttScope}
                  onChange={(e) => setTtScope(e.target.value)}
                  className="bg-white/[0.06] ring-1 ring-white/10 hover:ring-white/20 text-xs px-2 py-1 rounded-md outline-none"
                >
            ...
            <div className="p-4 space-y-3">
              <p className="text-[11px] text-slate-400">
                Add your class time blocks. Tasks can only be scheduled outside these blocks.
              </p>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          id={`tt-${day}-start`}
                          type="time"
                          className="bg-white/[0.06] ring-1 ring-white/10 text-xs rounded-md px-2 py-1 outline-none"
                        />
                        ...
                        />
                        <button
                          onClick={() => handleAddTimetableBlock(day)}
                          className="text-xs px-2 py-1 rounded-md bg-white/[0.06] hover:bg-white/[0.1] ring-1 ring-white/10"
                        >
                          Add
                        </button>
                      </div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {ttData[day].length > 0 ? (
                        ttData[day].map((block, index) => (
                          <span
                            key={index}
                            className="inline-flex items-center gap-2 text-xs px-2 py-1 rounded-md bg-white/[0.06] ring-1 ring-white/10"
                          >
                            ...
                            <button
                              onClick={() => handleDeleteTimetableBlock(day, index)}
              ...
              <div className="pt-2 border-t border-white/5">
                <p className="text-[11px] text-slate-500">
                  This applies only to the selected week. Switch to “Next week” to set a different timetable.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setNewCommonBlock((prev) => ({ ...prev, start: e.target.value }))
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewCommonBlock((prev) => ({ ...prev, end: e.target.value }))
                  }
                  className="w-full bg-white/[0.06] ring-1 ring-white/10 text-xs rounded-md px-2 py-1 outline-none"
                />
              </div>
                    ...
                    <button
                      ...
                      className={`text-xs px-2 py-1 rounded-md ring-1 ${
                        newCommonBlock.days.includes(day)
                          ? "bg-indigo-500/20 ring-indigo-400/20"
              ...
              <div className="mt-2 space-y-2">
                {commonBlocks.map((block) => (
                  <CommonBlockItem key={block.id} block={block} />
                ))}
              </div>
            </div>
          </div>
